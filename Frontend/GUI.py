...
def QueryModifier(Query):
    ...
    if any (word + "" in new_query for word in question_words): # if wh word is present then add ? to it
...
# writes the microphone's status in Mic.data
...
def GetMicrophoneStatus():
    try:
        ...
        return "Error: Mic data file not found"
    except Exception as e:
        return f"Error: {str(e)}"

# writes the assitant's working status in Status.data
def SetAssistantStatus(Status):
    with open(rf'{TempDirPath}\Status.data', "w", encoding='utf-8') as file:
        file.write(Status)

# SetAssistantStatus("Speaking...")
# returns assistant's working status
...
# Helper Functions
# returns graphics dir for images and icons
def GraphicsDirectoryPath(Filename):
    Path = rf'{GraphicsDirPath}\{Filename}' 
    return Path

# returns temp dir for data
def TempDirectoryPath(Filename):
    Path = rf'{TempDirPath}\{Filename}' 
    return Path

# to show the chat on screen
...
class InitialScreen(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        max_gif_size_W = 600  # Set desired width
        max_gif_size_H = 600  # Set desired height
        movie.setScaledSize(QSize(max_gif_size_W, max_gif_size_H))
